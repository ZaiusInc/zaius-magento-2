    ...
    public function sync($product)
    {
        try {
            ...
            foreach ($trackingIds as $trackingId => $storeIds) {
                if (sizeof($storeIds) > 0 && $product->getStoreId() != Store::DEFAULT_STORE_ID) {
                    foreach ($storeIds as $storeId) {
                        $scopeProduct = $this->productRepository->getById($product->getId(), false, $storeId);

                        if ($this->hasVariants($baseProduct, $scopeProduct) && $storeId == $product->getStoreId()) {
                ...
                } else if(sizeof($storeIds) > 0 && $product->getStoreId() == Store::DEFAULT_STORE_ID) {
                    //main product
